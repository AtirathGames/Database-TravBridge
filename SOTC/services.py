...
def get_new_auth_token() -> Tuple[str, str]:
    ...
    FIX FOR 406 ERROR: Uses a FRESH session instead of the shared api_session.
    The SOTC API firewall (AppTrana) blocks repeated requests on the same session
    as suspicious behavior. Each token request must come from a fresh session.
    ...
    """
    url = "https://services.sotc.in/commonRS/extnrt/getNewRequestToken"
    headers = COMMON_HEADERS  # already has Accept: */*, User-Agent, etc.

    try:
        # Create fresh session for token request to avoid WAF blocking
        fresh_session = requests.Session()
        try:
            logging.debug(f"Fetching auth token from {url}")
            resp = fresh_session.get(url, headers=headers, timeout=10)

            if resp.status_code == 406:
                logging.error(
                    f"406 Not Acceptable from SOTC API. WAF blocked the token request."
                )
                logging.error(f"Response: {resp.text[:300]}")
                raise RuntimeError(
                    "SOTC API firewall rejected token request (406). "
                    "This indicates the firewall is blocking the request as suspicious. "
            ...
            if data.get("errorCode") != 0:
                ...
                raise RuntimeError(f"SOTC API returned error: {error_msg}")

            # Extract and validate token
            request_id = data.get("requestId")
            session_id = data.get("tokenId")

            if not request_id or not session_id:
                logging.error(f"Invalid token response structure: {data}")
                raise RuntimeError("SOTC API returned invalid token response")
        ...
        raise RuntimeError("Timeout fetching auth token from SOTC API")
    except requests.exceptions.RequestException as e:
        logging.error(
            f"Request error while fetching auth token: {type(e).__name__}: {e}"
        )
        raise RuntimeError(f"Failed to fetch auth token: {e}")
    except Exception as e:
        logging.error(f"Unexpected error fetching auth token: {type(e).__name__}: {e}")
        raise RuntimeError(f"Auth token fetch failed: {e}")
...
async def index_package_in_es(package_data: dict):
    ...
    es.index(index=SOTC_PACKAGE_INDEX, id=doc_id, document=document)
    logging.info(f"Indexed package {doc_id} into {SOTC_PACKAGE_INDEX}")
...
async def search_item_by_package_id_internal(
    ...
    index = SOTC_PACKAGE_INDEX  # Adjust to your index name
...
def format_and_sort_months(month_year_list):
    ...
    for month_year in month_year_list:
        ...
        # Capitalize the month and format as "Month-Year"
        formatted_months.append(f"{month.capitalize()}-{year}")

    # Sort by month order
    formatted_months.sort(key=lambda x: month_order.index(x.split("-")[0].lower()))
...
def get_days_tier(package_days: int, target_days: int) -> int:
    ...
    if not target_days or target_days <= 0:
        return 1  # Treat as D1 if days not provided
...
def fetch_fare_calendar_git(
    ...
    """
    url = "https://services.sotc.in/holidayRS/pdp.compare/fareCalender"
    headers = {
    ...
    def fetch_city_data(city):
        ...
        try:
            ...
            logging.info(
                f"[fetch_fare_calendar_git] Request for {city.cityName}: {payload}"
            )
            response = requests.post(url, json=payload, headers=headers, timeout=30)
            response.raise_for_status()
            data = response.json()
            logging.info(
                f"[fetch_fare_calendar_git] Response for {city.cityName}: {data}"
            )
