...
@router.post("/v1/save_conversation")
...
@router.post("/v1/get_conversation")
def get_conversation(body: ConversationIdRequest):
    ...
    try:
        if not es.indices.exists(index=CHAT_INDEX_NAME):
            raise HTTPException(
                status_code=404, detail=f"Index '{CHAT_INDEX_NAME}' not found."
            )
...
@router.post("/v1/get_conversation_summaries")
...
@router.post("/v1/get_packages_saved")
...
@router.post("/v1/update_chat_name")
...
@router.post("/v1/delete_conversation")
...
@router.post("/v1/get_user_conversations")
...
@router.post("/v1/get_all_conversations")
...
# ✅ API Route
@router.post("/v1/export_conversations")
def export_conversation_summary(request: DateRangeRequest):
    try:
        ...
        scroll = es.search(
            index=CHAT_INDEX_NAME, body={"query": query}, scroll="2m", size=1000
        )
        ...
        exact_assistant_content = '{"message": "Hi, I am Tacy, your personal Gen AI holiday assistant from Thomas Cook. Where would you like to travel?", "name": "packageSearch", "arguments": {"searchText": "", "dates": {"start": "", "end": ""}, "number_of_people": null, "number_of_days": null, "budget": "", "hub": "", "first_name": "", "last_name": "", "email": "", "phone": ""}}'
