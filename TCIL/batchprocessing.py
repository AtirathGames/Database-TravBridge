...
from constants import MAX_RETRIES, TIMEOUT, es, TCIL_PACKAGE_INDEX, TCIL_RAW_INDEX, ENABLE_SCHEDULED_JOB
...
from scrape_destinations_batch import run_destination_scraping
...
def reset_index():
    ...
    """
    if es.indices.exists(index=TCIL_PACKAGE_INDEX):
        logging.info(f"Deleting existing index: {TCIL_PACKAGE_INDEX}")
        es.indices.delete(index=TCIL_PACKAGE_INDEX)

    logging.info(f"Creating index: {TCIL_PACKAGE_INDEX} with correct mapping")
    es.indices.create(index=TCIL_PACKAGE_INDEX, body=package_index_mapping)
...
async def batch_process_all_packages(background_tasks: BackgroundTasks):
    ...
    3. Fetches package details in parallel and indexes them into `TCIL_PACKAGE_INDEX`.
...
async def clear_index():
    """
    Delete all documents in both TCIL_PACKAGE_INDEX and TCIL_RAW_INDEX.
    """
    for index in [TCIL_PACKAGE_INDEX, TCIL_RAW_INDEX]:
        try:
            exists = await asyncio.to_thread(es.indices.exists, index=index)
            if not exists:
                logging.info(f"Index {index} does not exist, skipping clear.")
                continue
            logging.info(f"Clearing all documents in index: {index}...")
            ...
            logging.error(f"Error clearing index {index}: {str(e)}")
            raise
...
async def process_all_packages(months: List[str], current_year: int):
    ...
    5. Fetches package details and indexes them into `TCIL_PACKAGE_INDEX`.
...
async def process_unique_packages(package_months: dict):
    """
    Fetches package details concurrently for unique package IDs and stores them in the main index (`TCIL_PACKAGE_INDEX`).
    ...
    async def limited_fetch_and_store(package_id):
        async with semaphore:
            ...
            if isinstance(result, tuple):
                processed_action, raw_action = result
                summary = processed_action.get("doc", {}).get("packageSummary")
    ...
    if es_actions:
        try:
            ...
            logging.info(f"📌 Successfully indexed {success} packages into {TCIL_PACKAGE_INDEX}. Failed: {failed}")

            # Bulk index raw PDP responses into tcildatav1_raw
            if es_raw_actions:
                try:
                    raw_success, raw_failed = await asyncio.to_thread(
                        helpers.bulk, es, es_raw_actions, chunk_size=500, stats_only=True
                    )
                    logging.info(f"📌 Successfully indexed {raw_success} raw documents into {TCIL_RAW_INDEX}. Failed: {raw_failed}")
                except Exception as raw_e:
                    logging.error(f"❌ Error during raw bulk indexing: {str(raw_e)}")
    ...
    try:
        ...
        processed_action = {
            ...
            "doc": {
                ...
                "serviceSlots": package_data.get("serviceSlots"),
                "sightseeingTypes": package_data.get("sightseeingTypes", [])
            },
            "doc_as_upsert": True
        }
...
async def ensure_index_exists():
    ...
    """
    try:
        exists = await asyncio.to_thread(es.indices.exists, index=TCIL_PACKAGE_INDEX)
        if not exists:
            logging.info(f"Index {TCIL_PACKAGE_INDEX} does not exist. Creating...")
            await asyncio.to_thread(es.indices.create, index=TCIL_PACKAGE_INDEX, body=package_index_mapping)
            logging.info(f"Index {TCIL_PACKAGE_INDEX} created.")
        else:
            logging.info(f"Index {TCIL_PACKAGE_INDEX} already exists.")
    except Exception as e:
...
async def ensure_raw_index_exists():
    """
    Checks if the raw PDP index exists; if not, creates it with minimal mapping.
    raw_response is stored as-is (enabled: false) — not indexed, only retrievable.
    """
    raw_index_mapping = {
        "mappings": {
            "properties": {
    ...
    try:
        exists = await asyncio.to_thread(es.indices.exists, index=TCIL_RAW_INDEX)
        if not exists:
            logging.info(f"Index {TCIL_RAW_INDEX} does not exist. Creating...")
            await asyncio.to_thread(es.indices.create, index=TCIL_RAW_INDEX, body=raw_index_mapping)
            logging.info(f"Index {TCIL_RAW_INDEX} created.")
        else:
            logging.info(f"Index {TCIL_RAW_INDEX} already exists.")
    except Exception as e:
...
# -----------------------------------------------------------------------
#  8) The APScheduler job to run at 00:30 IST daily & email stats
# -----------------------------------------------------------------------
async def run_scheduled_batch_process():
    ...
    try:
        # PHASE 1: Process packages (existing logic)
        logging.info("=" * 60)
        logging.info("PHASE 1: Processing Package Data")
        logging.info("=" * 60)
        package_result = await process_all_packages(months, this_year)
        
        # PHASE 2: Scrape destination FAQs
        logging.info("=" * 60)
        logging.info("PHASE 2: Scraping Destination FAQs")
        logging.info("=" * 60)
        scraping_result = await run_destination_scraping()
        
        # PHASE 3: Geocode new destinations (incremental)
        logging.info("=" * 60)
        logging.info("PHASE 3: Geocoding New Destinations")
        logging.info("=" * 60)
        try:
            from geocode_destinations import geocode_and_store_destinations
            geocoding_result = await geocode_and_store_destinations(incremental=True)
            logging.info(f"Geocoding completed: {geocoding_result}")
        except Exception as geo_error:
            logging.error(f"Geocoding phase failed: {geo_error}")
            geocoding_result = {"success": False, "error": str(geo_error)}
        
        # Send success email with all results
        body = (
            f"Hello,\n\n"
            f"The daily batch process has completed successfully.\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"PACKAGE PROCESSING RESULTS:\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"Failed Months: {', '.join(package_result['failed_months']) or 'None'}\n"
            f"Total Packages: {package_result['total_packages']}\n"
            f"Summary Generated: {package_result['summary_stats']['generated']}\n"
            f"Summary Failed: {package_result['summary_stats']['failed']}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"DESTINATION FAQ SCRAPING RESULTS:\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"Total Destinations: {scraping_result['total']}\n"
            f"Successfully Scraped: {scraping_result['scraped']}\n"
            f"Failed: {scraping_result['failed']}\n"
            f"FAQ Documents Indexed: {scraping_result['faq_documents']}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"DESTINATION GEOCODING RESULTS:\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        )
        
        if geocoding_result and geocoding_result.get('success'):
            body += (
                f"Total Destinations in Packages: {geocoding_result.get('total_cities', 0)}\n"
                f"New Destinations Found: {geocoding_result.get('new_cities', 0)}\n"
                f"Successfully Geocoded: {geocoding_result.get('geocoded', 0)}\n"
                f"Failed: {geocoding_result.get('failed', 0)}\n"
            )
            if geocoding_result.get('failed_cities'):
                failed_list = geocoding_result['failed_cities'][:10]
                body += f"Failed Cities: {', '.join(failed_list)}"
                if len(geocoding_result['failed_cities']) > 10:
                    body += f" (and {len(geocoding_result['failed_cities']) - 10} more)"
                body += "\n"
        else:
            body += f"Geocoding Status: {'Failed' if geocoding_result else 'Skipped'}\n"
            if geocoding_result and geocoding_result.get('error'):
                body += f"Error: {geocoding_result['error']}\n"
        
        body += "\n"
        
        if scraping_result['failed_destinations']:
            body += f"\nFailed Destinations: {', '.join(scraping_result['failed_destinations'][:10])}"
            if len(scraping_result['failed_destinations']) > 10:
                body += f" ... and {len(scraping_result['failed_destinations']) - 10} more"
        
        body += "\n\nRegards,\nBatch Process Scheduler"
        
        send_email_report("Daily Batch Processing Report - SUCCESS", body, TO_EMAILS)
        
    except Exception as e:
        logging.error(f"Batch process failed: {str(e)}")
        
        # Send failure email
        error_body = f"Hello,\n\nThe daily batch process has FAILED.\n\n"
        
        if package_result:
            error_body += (
                f"Package Processing: COMPLETED\n"
                f"  - Total Packages: {package_result['total_packages']}\n"
                f"  - Failed Months: {', '.join(package_result['failed_months']) or 'None'}\n\n"
            )
        else:
            error_body += "Package Processing: FAILED\n\n"
        
        if scraping_result:
            error_body += (
                f"Destination Scraping: COMPLETED\n"
                f"  - Scraped: {scraping_result['scraped']}/{scraping_result['total']}\n\n"
            )
        else:
            error_body += "Destination Scraping: FAILED\n\n"
        
        error_body += f"Error Details:\n{str(e)}\n\nRegards,\nBatch Process Scheduler"
        
        send_email_report("Daily Batch Processing FAILED", error_body, TO_EMAILS)
...
async def schedule_daily_job():
    ...
    scheduler.start()
    logging.info("Scheduler started: batch will run daily at 00:30 IST.")
...
@router.post("/scrape_destinations_now")
async def scrape_destinations_now(background_tasks: BackgroundTasks):
    """
    Manually trigger destination FAQ scraping only.
    Useful for testing or on-demand updates.
    """
    try:
        background_tasks.add_task(scrape_destinations_task)
        return {"message": "Destination FAQ scraping started in background."}
    except Exception as e:
        logging.error(f"Error starting destination scraping: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

async def scrape_destinations_task():
    """Background task for destination scraping with email notification."""
    try:
        logging.info("Starting manual destination scraping task...")
        result = await run_destination_scraping()
        
        body = (
            f"Hello,\n\n"
            f"Manual destination FAQ scraping has completed.\n\n"
            f"Total Destinations: {result['total']}\n"
            f"Successfully Scraped: {result['scraped']}\n"
            f"Failed: {result['failed']}\n"
            f"FAQ Documents Indexed: {result['faq_documents']}\n"
        )
        
        if result['failed_destinations']:
            body += f"\nFailed Destinations: {', '.join(result['failed_destinations'][:20])}"
            if len(result['failed_destinations']) > 20:
                body += f" ... and {len(result['failed_destinations']) - 20} more"
        
        body += "\n\nRegards,\nDestination Scraping Service"
        
        send_email_report("Destination FAQ Scraping Report - SUCCESS", body, TO_EMAILS)
        
    except Exception as e:
        logging.error(f"Destination scraping task failed: {str(e)}")
        send_email_report(
            "Destination FAQ Scraping FAILED",
            f"Error: {str(e)}",
            TO_EMAILS
        )