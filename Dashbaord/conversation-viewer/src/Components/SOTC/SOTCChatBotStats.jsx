...
const FunnelBars = ({ title, data }) => {
  ...
  return (
    <div className="sotc-funnel-bars">
      <div className="sotc-funnel-bars-title">{title}</div>
      {data.map((d, i) => {
        const pct = max ? Math.round((d.value / max) * 100) : 0;
        return (
          <div key={i} className="sotc-funnel-row">
            <div className="sotc-funnel-label">{d.label}</div>
            <div className="sotc-funnel-track">
              <div
                className="sotc-funnel-fill"
                style={{ width: `${pct}%`, backgroundColor: d.color }}
              />
            </div>
            <div className="sotc-funnel-value">{d.value}</div>
          </div>
        );
      })}
    </div>
...
const StepStackedFunnel = ({ title, stages }) => {
  ...
  return (
    <div className="sotc-stacked-funnel">
      <div className="sotc-stacked-funnel-header">{title}</div>
      <div className="sotc-stacked-funnel-body">
...
function SOTCChatBotStats() {
  ...
  // series by date (if you ever expose split/funnels later)
  ...
  return (
    <div className="sotc-cb-container">
      {/* Header */}
      <div className="sotc-cb-header-row">
        <button
          onClick={() => window.history.back()}
          className="sotc-cb-btn sotc-cb-btn-light"
        >
          ← Back
        </button>
        <div className="sotc-cb-header">Conversation Insights — ChatBot (SOTC)</div>
        <span className="sotc-cb-chip">Active</span>
      </div>

      {/* Controls */}
      <div className="sotc-cb-controls">
        <div className="sotc-cb-date-row">
          <input
            type="date"
            className="sotc-cb-date-input"
            value={fromDate}
        ...
        <div>
          <button className="sotc-cb-btn" onClick={downloadExcel}>
            Download Excel
          </button>
        </div>
      </div>
      ...
      {/* Overview */}
      <div className="sotc-cb-section">
        <div className="sotc-cb-section-title">Overview</div>
        <div className="sotc-cb-kpi-grid">
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Total Conversations</div>
            <div className="sotc-cb-card-value">{summary.total}</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Opportunities</div>
            <div className="sotc-cb-card-value">{summary.opportunities}</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Empty Conversations</div>
            <div className="sotc-cb-card-value">{summary.empty}</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Overall Conversion</div>
            <div className="sotc-cb-card-value">{overallCv}%</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Empty Conversations %</div>
            <div className="sotc-cb-card-value">{emptyConvoPct}%</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Conversations → Leads %</div>
            <div className="sotc-cb-card-value">{convToLeadsPct}%</div>
          </div>
        </div>
      </div>

      {/* Live agent */}
      <div className="sotc-cb-section sotc-cb-section-margin">
        <div className="sotc-cb-section-title">Live Agent Transfers</div>
        <div className="sotc-cb-kpi-grid-auto">
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Live — Total</div>
            <div className="sotc-cb-card-value">{liveTotals.total}</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Success</div>
            <div className="sotc-cb-card-value">{liveTotals.success}</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Agents Unavailable</div>
            <div className="sotc-cb-card-value">{liveTotals.not_available}</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Agent Not Accepted</div>
            <div className="sotc-cb-card-value">{liveTotals.agent_not_accepted}</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Agents Busy</div>
            <div className="sotc-cb-card-value">{liveTotals.agent_busy}</div>
          </div>
          <div className="sotc-cb-card">
            <div className="sotc-cb-card-title">Requested Only</div>
            <div className="sotc-cb-card-value">{liveTotals.requested_only}</div>
          </div>

          <div className="sotc-cb-card sotc-cb-live-card-flex">
            <div>
              <div className="sotc-cb-card-title">Details</div>
              <div className="sotc-cb-card-subtitle">View transfer-level rows</div>
            </div>
            <button
              className="sotc-cb-btn sotc-cb-btn-primary"
              onClick={() => {
                setPage(1);
                setLiveOpen(true);
              }}
      ...
      <div className="sotc-cb-main-grid">
        <div className="sotc-cb-charts-wrap">
          <div className="sotc-cb-charts-header">
            <div className="sotc-cb-charts-title">Charts</div>
            <div className="sotc-cb-charts-controls">
              {/* For now, only Trend tab is visible like your original */}
              <div className="sotc-cb-pills">
                <div
                  className={`sotc-cb-pill ${tab === "trend" ? "active" : ""}`}
          ...
          {loading ? (
            <div className="sotc-cb-spinner">Loading...</div>
          ) : error ? (
            <div className="sotc-cb-error-text">{error}</div>
          ) : (
            <>
              {tab === "trend" && (
        ...
        {/* Top Destinations */}
        <div className="sotc-cb-top-dest-wrapper">
          {topDestinations?.length > 0 && (
            <div className="sotc-cb-side-card">
              <div className="sotc-cb-side-card-title">Top Destinations</div>
              <ul className="sotc-cb-top-dest-list">
      ...
      {/* Live details modal */}
      {liveOpen && (
        <div className="sotc-cb-modal-backdrop">
          <div className="sotc-cb-modal">
            <div className="sotc-cb-modal-header">
              <div>
            ...
            <div className="sotc-cb-modal-body">
              <div className="sotc-cb-table-container">
                <table className="sotc-cb-table">
                  <thead>
                    <tr>
                      {[
                        "Date",
                  ...
                  <tbody>
                    {pageRows.map((row, idx) => (
                      <tr key={start + idx}>
                        <td className="sotc-cb-td">{toISTDate(row["Date"])}</td>
                        <td className="sotc-cb-td">
                          {row["Conversation ID"] || "—"}
                        </td>
                        <td className="sotc-cb-td">
                          {titleize(row["User Type"])}
                        </td>
                        <td className="sotc-cb-td">{row["Agent ID"] || "—"}</td>
                        <td className="sotc-cb-td">
                          {row["Attended Agents"] || "—"}
                        </td>
                        <td className="sotc-cb-td">
                          <span style={statusPillStyle(row["Final Status"])}>
                    ...
                    {pageRows.length === 0 && (
                      <tr>
                        <td className="sotc-cb-td sotc-cb-td-muted" colSpan={14}>
                          No live transfer chats in this range.
                        </td>
                      </tr>
                    )}
                    <tr>
                      <td colSpan={14} className="sotc-cb-td sotc-cb-td-footer">
                        <div className="sotc-cb-pagination-footer">
                          <div className="sotc-cb-pagination-info">
                            Showing{" "}
                            {sorted.length === 0 ? 0 : start + 1}–
                            {Math.min(start + pageSize, sorted.length)} of{" "}
                            {sorted.length}
                          </div>
                          <div className="sotc-cb-pagination-buttons">
                            <button
                              className="sotc-cb-btn"
                              disabled={safePage <= 1}
                              onClick={() =>
                            ...
                            <span className="sotc-cb-page-indicator">
...
export default SOTCChatBotStats;
